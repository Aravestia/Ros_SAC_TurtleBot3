...
class SacEnvV4(gym.Env):
    def __init__(self, amr_model='turtlebot3_burger', epoch=0, init_positions=[], stage_map="", yaw=0.0, max_timesteps=10000, test_mode=False):
        ...
        self.waypoint_min_distance_threshold = 0.25
        ...
        # Action space: [velocity, angular velocity, follower mode]
    ...
    def step(self, action):
        ...
        self.follower_mode = float(action[2]) > 0
        ...
        if not self.follower_mode:
            ...
            dampen_velocity = 0.2
            velocity = (1 - dampen_velocity + (dampen_velocity * float(action[0])))

            turning_rate = 0.4 + (0.1 * float(action[1]))
            angle_threshold = 0.15

            if self.waypoint_closest_angle < -angle_threshold:
                self.angular_velocity = -abs(turning_rate)
                self.velocity = 0.5 * velocity
            elif self.waypoint_closest_angle > angle_threshold:
                self.angular_velocity = abs(turning_rate)
                self.velocity = 0.5 * velocity
            else:
                self.angular_velocity = 0.2 * turning_rate
    ...
    def _compute_reward(self):
        ...
        if laserscan_closest <= warning_threshold and abs(laserscan_closest_angle) < laserscan_angle_threshold:
            if velocity > velocity_threshold:
                reward_velocity =  (1 - velocity) / (1 - velocity_threshold)
            else:
                reward_velocity = (1 + velocity) / (1 + velocity_threshold)

            penalty_facing_obstacle = -(laserscan_angle_threshold - abs(laserscan_closest_angle)) / laserscan_angle_threshold
            reward_waypoint = 0
            penalty_high_turning = 0
        ...
        if self.waypoint_closest >= len(self.waypoints) - 1 and distance_from_waypoint < waypoint_distance_threshold: # Reached Goal
        ...
        if self.step_count >= self.max_step_count: # Maxed Step Count
           reward += 4.0 * penalty_step_count_maxed
           self.end_episode()
           return float(reward)
